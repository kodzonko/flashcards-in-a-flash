...
async def test_process_dataframe_async():
    test_df = pd.DataFrame({"text": ["Hello world", "This is a test"]})
...
async def test_generate_audio():
    ...
    audio_bytes = await generate_audio(text, locale)
...
async def test_process_row():
    ...
    with patch(
        "flashcards_in_a_flash.audio_generator.generate_audio", new_callable=AsyncMock
    ) as mock_generate_audio:
        mock_audio = b"dummy audio data"
        mock_generate_audio.return_value = mock_audio

        audio_result = await process_row(row, "en-US")
